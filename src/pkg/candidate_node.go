...
func (n *CandidateNode) GetValueRep() (interface{}, error) {
	clog.Debug("GetValueRep for %v value: %v", n.GetNicePath(), n.Value)
...
func (n *CandidateNode) guessTagFromCustomType() string {
	if strings.HasPrefix(n.Tag, "!!") {
		...
		clog.Debug("guessTagFromCustomType: node has no value to guess the type with")
		return n.Tag
	}
	dataBucket, errorReading := parseSnippet(n.Value)

	if errorReading != nil {
		clog.Debug("guessTagFromCustomType: could not guess underlying tag type %v", errorReading)
		return n.Tag
	}
	guessedTag := dataBucket.Tag
	clog.Info("im guessing the tag %v is a %v", n.Tag, guessedTag)
	return guessedTag
}
