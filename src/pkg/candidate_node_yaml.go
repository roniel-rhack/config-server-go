...
func (o *CandidateNode) copyFromYamlNode(node *yaml.Node, anchorMap map[string]*CandidateNode) {
	...
	if o.Anchor != "" {
		...
		clog.Debug("set anchor %v to %v", o.Anchor, NodeToString(o))
	}

	// its a single alias
	if node.Alias != nil && node.Alias.Anchor != "" {
		o.Alias = anchorMap[node.Alias.Anchor]
		clog.Debug("set alias to %v", NodeToString(anchorMap[node.Alias.Anchor]))
	}
...
func (o *CandidateNode) UnmarshalYAML(node *yaml.Node, anchorMap map[string]*CandidateNode) error {
	clog.Debug("UnmarshalYAML %v", node.Tag)
	switch node.Kind {
	case yaml.AliasNode:
		clog.Debug("UnmarshalYAML - alias from yaml: %v", o.Tag)
		o.Kind = AliasNode
		o.copyFromYamlNode(node, anchorMap)
		return nil
	case yaml.ScalarNode:
		clog.Debug("UnmarshalYAML -  a scalar")
		o.Kind = ScalarNode
		o.copyFromYamlNode(node, anchorMap)
		return nil
	case yaml.MappingNode:
		clog.Debug("UnmarshalYAML -  a mapping node")
		o.Kind = MappingNode
		o.copyFromYamlNode(node, anchorMap)
		...
		clog.Debug("UnmarshalYAML -  finished mapping node")
		return nil
	case yaml.SequenceNode:
		clog.Debug("UnmarshalYAML -  a sequence: %v", len(node.Content))
		o.Kind = SequenceNode

		o.copyFromYamlNode(node, anchorMap)
		clog.Debug("node Style: %v", node.Style)
		clog.Debug("o Style: %v", o.Style)
		o.Content = make([]*CandidateNode, len(node.Content))
	...
	case 0:
		...
		o.copyFromYamlNode(node, anchorMap)
		clog.Debug("UnmarshalYAML -  err.. %v", NodeToString(o))
		return nil
	default:
...
func (o *CandidateNode) MarshalYAML() (*yaml.Node, error) {
	clog.Debug("MarshalYAML to yaml: %v", o.Tag)
	switch o.Kind {
	case AliasNode:
		clog.Debug("MarshalYAML - alias to yaml: %v", o.Tag)
		target := &yaml.Node{Kind: yaml.AliasNode}
		o.copyToYamlNode(target)
		return target, nil
	case ScalarNode:
		clog.Debug("MarshalYAML - scalar: %v", o.Value)
	...
	case MappingNode, SequenceNode:
		...
		o.copyToYamlNode(target)
		clog.Debug("original style: %v", o.Style)
		clog.Debug("original: %v, tag: %v, style: %v, kind: %v", NodeToString(o), target.Tag, target.Style, target.Kind == yaml.SequenceNode)
