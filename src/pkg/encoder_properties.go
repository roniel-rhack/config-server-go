...
func (pe *propertiesEncoder) EncodeToMap(node *CandidateNode) (map[string]string, error) {
	// Create a new properties object
	p := properties.NewProperties()
	p.WriteSeparator = pe.prefs.KeyValueSeparator

	// Use the doEncode function to populate the properties object
	err := pe.doEncode(p, node, "", nil)
	if err != nil {
		return nil, err
	}

	// Create a map to hold the keys and values from the properties object
	resultMap := make(map[string]string)

	// Iterate over the properties object and populate the map
